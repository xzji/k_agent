...
exports.UnifiedTaskScheduler = void 0;
...
class UnifiedTaskScheduler {
    ...
    constructor(taskStore, goalStore, knowledgeStore, notificationQueue, dependencyGraph, executionPipeline, idleDetector, userProfile, valueAssessor, config) {
        ...
        this.semaphore = new utils_1.Semaphore(this.config.maxConcurrent);
    }
    /**
    ...
    async runCycle() {
        ...
        const cycleStart = (0, utils_1.now)();
        try {
        ...
        const cycleDuration = (0, utils_1.now)() - cycleStart;
    ...
    async getExplorationTasks() {
        ...
        for (const goal of activeGoals) {
            ...
            for (const dim of pendingDims) {
                ...
                const hasExplorationTask = existingTasks.some((t) => t.type === 'exploration' && t.dimensionId === dim.id &&
     ...
     */
    getPendingDimensions(goal) {
        return goal.dimensions.filter((d) => d.status === 'pending' || d.status === 'exploring');
    }
    /**
     * Calculate priority for a dimension
     */
    calculateDimPriority(dim, goal) {
     ...
     */
    async updateBlockedTaskStatuses() {
     ...
     */
    async filterExecutableTasks(tasks) {
        const executable = [];
     ...
     */
    async isGoodTimeToExecute(task) {
    ...
    prioritizeTasks(tasks) {
        return tasks.sort((a, b) => {
            const weightA = (0, utils_1.getPriorityWeight)(a.priority);
            const weightB = (0, utils_1.getPriorityWeight)(b.priority);
            if (weightA !== weightB) {
     ...
     */
    async executeWithConcurrency(tasks) {
     ...
     */
    async executeUnifiedTask(unifiedTask) {
     ...
     */
    async runTaskWithState(unifiedTask) {
     ...
     */
    async createExplorationTask(unifiedTask) {
     ...
     */
    async executeExplorationTask(task) {
     ...
     */
    async executeRecurringTask(task) {
     ...
     */
    async executeOneTimeTask(task) {
        return this.executionPipeline.run(task);
    }
    /**
     * Execute interactive task
     */
    async executeInteractiveTask(task) {
     ...
     */
    async executeEventTriggeredTask(task) {
     ...
     */
    generateVariation(basePrompt, history) {
...
`;
     ...
     */
    adjustDifficulty(prompt, history) {
...
[Difficulty Adjustment: User is performing well, can increase depth and challenge]`;
     ...
     */
    async scheduleNextExecution(task) {
        if (!task.schedule)
            return;
        ...
        const nextExecutionAt = (0, utils_1.now)() + schedule.intervalMs;
     ...
     */
    async handleAdaptiveAdjustments(task, result) {
     ...
     */
    async assessAndNotify(task, result) {
        try {
            // Assess value
            const assessment = await this.valueAssessor.assessValue(task.id, result);
            // Log assessment
            console.log(`[Scheduler] Task ${task.id} value score: ${assessment.valueScore}, notify: ${assessment.shouldNotify}`);
     ...
     */
    async waitForUserIdle(timeoutMs = 300000) {
        const checkInterval = 10000; // 10 seconds
        const startTime = (0, utils_1.now)();
        while ((0, utils_1.now)() - startTime < timeoutMs) {
            if (await this.idleDetector.isUserIdle()) {
                return true;
            }
            await (0, utils_1.sleep)(checkInterval);
        }
     ...
     */
    buildNotificationContent(task, result, assessment) {
        const lines = [];
        // Result output
        lines.push('## 执行结果');
     ...
     */
    enqueueResultNotification(task, result) {
        const notification = {
            type: 'report',
     ...
     */
    async handleUserResponse(taskId, response) {
...
exports.UnifiedTaskScheduler = UnifiedTaskScheduler;
