...
class SubGoalPlanner {
    ...
    subGoalStore;
    llm;
    constructor(goalStore, subGoalStore, llm) {
     ...
     */
    async decomposeSubGoals(goalId, userContext) {
        ...
        const tempIdMap = new Map(); // Map temp IDs to real IDs
     ...
     */
    async createDefaultSubGoals(goal) {
        const defaults = [
            {
                goalId: goal.id,
                name: '信息收集与调研',
     ...
     */
    async reviewSubGoals(goalId, taskStore) {
        const goal = await this.goalStore.getGoal(goalId);
        if (!goal) {
            throw new Error(`Goal not found: ${goalId}`);
        }
        ...
        const reviews = reviewData.reviewResults.map((r) => ({
     ...
     */
    async replanSubGoals(goalId, feedback, userContext) {
     ...
     */
    async canActivateSubGoal(subGoalId) {
     ...
     */
    async updateSubGoalStatusFromTasks(subGoalId) {
        const subGoal = await this.subGoalStore.getSubGoal(subGoalId);
        if (!subGoal) {
            throw new Error(`SubGoal not found: ${subGoalId}`);
        }
...
exports.SubGoalPlanner = SubGoalPlanner;
/**
 * Generate unique ID
 */
function generateId() {
...
//# sourceMappingURL=sub-goal-planner.js.map