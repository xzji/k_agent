...
exports.SimpleIdleDetector = exports.LocalExecutionPipeline = exports.ClaudeLLMChannel = void 0;
/**
 * Claude LLM Channel - 直接实现，无需外部 API
 */
class ClaudeLLMChannel {
    context = new Map();
    /**
     ...
     * 生成子目标拆解
     */
    generateSubGoalDecomposition(prompt) {
        // 提取目标标题
     ...
     * 创建默认子目标
     */
    createDefaultSubGoals(goalTitle) {
        // 根据目标关键词判断类型
     ...
     * 创建默认任务
     */
    createDefaultTasks(subGoalName) {
     ...
     * 生成价值评估
     */
    generateValueAssessment(prompt) {
        // 基于内容质量给出合理评估
     ...
     * 生成对话响应
     */
    generateChatResponse(content, systemPrompt) {
        // 问题生成
     ...
     * 提取已收集的信息
     */
    extractCollectedInfo(content) {
        const info = {};
        // 提取预算
        const budgetMatch = content.match(/(\d+\s*万|\d+\s*元|\d+\s*\$)/);
        if (budgetMatch)
            info.预算 = budgetMatch[1];
        // 提取时间
...
exports.ClaudeLLMChannel = ClaudeLLMChannel;
/**
 * 本地执行管道 - 直接执行，无需外部服务
 */
class LocalExecutionPipeline {
    async run(task) {
    ...
    generateExplorationOutput(task) {
...
exports.LocalExecutionPipeline = LocalExecutionPipeline;
/**
 * 简单空闲检测器
 */
class SimpleIdleDetector {
...
exports.SimpleIdleDetector = SimpleIdleDetector;
//# sourceMappingURL=claude-llm-adapter.js.map