...
exports.runBuyCarDemo = runBuyCarDemo;
...
async function runBuyCarDemo() {
    ...
    const taskStore = new index_1.TaskStore();
    const knowledgeStore = new index_1.KnowledgeStore();
    const subGoalStore = new index_1.SubGoalStore();
    const notificationQueue = new index_1.NotificationQueue();
    await goalStore.init();
    await taskStore.init();
    await knowledgeStore.init();
    ...
    // 创建 LLM 适配器（使用本地模拟）
    const llm = new index_1.ClaudeLLMChannel();
    const idleDetector = new index_1.SimpleIdleDetector();
    // 步骤1: 创建目标
    console.log('\n📌 步骤1: 用户提交购车目标');
    ...
    // 步骤3: 子目标拆解
    console.log('\n📌 步骤3: 拆解子目标');
    const subGoalPlanner = new index_1.SubGoalPlanner(goalStore, subGoalStore, llm);
    ...
    // 步骤4: 任务生成
    console.log('\n📌 步骤4: 生成任务');
    const taskPlanner = new index_1.TaskPlanner(taskStore, subGoalStore, llm);
    const allTasks = [];
    for (const subGoal of subGoals.slice(0, 3)) { // 只为前3个子目标生成任务
        const tasks = await taskPlanner.generateTasksForSubGoal(subGoal.id, {
    ...
    // 步骤5: 计划展示
    console.log('\n📌 步骤5: 生成计划报告');
    const planPresenter = new index_1.PlanPresenter(goalStore, subGoalStore, taskStore, notificationQueue, llm);
    ...
    // 步骤6: 创建执行管道和调度器
    console.log('\n📌 步骤6: 初始化执行系统');
    const executionPipeline = new index_1.ExecutionPipeline(llm, knowledgeStore, taskStore);
    const dependencyGraph = new index_1.TaskDependencyGraph(taskStore);
    // 创建 ValueAssessor（需要模拟）
    const valueAssessor = {
        assessValue: async (taskId, result) => ({
            taskId,
            valueScore: 0.75,
            valueDimensions: {
                relevance: 0.8,
    ...
    const scheduler = new index_1.UnifiedTaskScheduler(taskStore, goalStore, knowledgeStore, notificationQueue, dependencyGraph, executionPipeline, idleDetector, {
        preferences: {
            notificationFrequency: 'immediate',
        },
    }, valueAssessor, { maxConcurrent: 2, cycleIntervalMs: 1000 });
