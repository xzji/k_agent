...
function color256ToHex(index) {
    ...
    if (index < 232) {
        ...
        var b = cubeIndex % 6;
        var toComponent_1 = function (n) { return (n === 0 ? 0 : 55 + n * 40); };
        var toHex = function (n) { return toComponent_1(n).toString(16).padStart(2, "0"); };
...
function styleToInlineCSS(style) {
    var parts = [];
    if (style.fg)
        parts.push("color:".concat(style.fg));
    if (style.bg)
        parts.push("background-color:".concat(style.bg));
    if (style.bold)
    ...
    while (i < params.length) {
        ...
        if (code === 0) {
            ...
            if (params[i + 1] === 5 && params.length > i + 2) {
                ...
                var g = params[i + 3];
                var b = params[i + 4];
                style.fg = "rgb(".concat(r, ",").concat(g, ",").concat(b, ")");
                i += 4;
            }
        }
        else if (code === 39) {
            ...
            if (params[i + 1] === 5 && params.length > i + 2) {
                ...
                var g = params[i + 3];
                var b = params[i + 4];
                style.bg = "rgb(".concat(r, ",").concat(g, ",").concat(b, ")");
                i += 4;
            }
        }
        else if (code === 49) {
...
function ansiToHtml(text) {
    var style = createEmptyStyle();
    var result = "";
    var lastIndex = 0;
    var inSpan = false;
    // Reset regex state
    ANSI_REGEX.lastIndex = 0;
    var match = ANSI_REGEX.exec(text);
    while (match !== null) {
        ...
        var params = paramStr ? paramStr.split(";").map(function (p) { return parseInt(p, 10) || 0; }) : [0];
        ...
        if (hasStyle(style)) {
            result += "<span style=\"".concat(styleToInlineCSS(style), "\">");
            inSpan = true;
        }
...
function ansiLinesToHtml(lines) {
    return lines.map(function (line) { return "<div class=\"ansi-line\">".concat(ansiToHtml(line) || "&nbsp;", "</div>"); }).join("\n");
}
