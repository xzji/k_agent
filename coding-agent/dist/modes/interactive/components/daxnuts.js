"use strict";
/**
 * POWERED BY DAXNUTS - Easter egg for OpenCode + Kimi K2.5
 *
 * A heartfelt tribute to dax (@thdxr) for providing free Kimi K2.5 access via OpenCode.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.DaxnutsComponent = void 0;
var theme_js_1 = require("../theme/theme.js");
// 32x32 RGB image of dax, hex encoded (3 bytes per pixel)
var DAX_HEX = "";
var WIDTH = 32;
var HEIGHT = 32;
function parseImage() {
    var pixels = [];
    for (var y = 0; y < HEIGHT; y++) {
        var row = [];
        for (var x = 0; x < WIDTH; x++) {
            var idx = (y * WIDTH + x) * 6;
            var r = parseInt(DAX_HEX.slice(idx, idx + 2), 16);
            var g = parseInt(DAX_HEX.slice(idx + 2, idx + 4), 16);
            var b = parseInt(DAX_HEX.slice(idx + 4, idx + 6), 16);
            row.push([r, g, b]);
        }
        pixels.push(row);
    }
    return pixels;
}
function rgb(r, g, b, bg) {
    if (bg === void 0) { bg = false; }
    return "\u001B[".concat(bg ? 48 : 38, ";2;").concat(r, ";").concat(g, ";").concat(b, "m");
}
var RESET = "\x1b[0m";
function buildImage() {
    var _a, _b;
    var pixels = parseImage();
    var lines = [];
    // Use half-block chars: ▄ with bg=top pixel, fg=bottom pixel
    for (var row = 0; row < HEIGHT; row += 2) {
        var line = "";
        for (var x = 0; x < WIDTH; x++) {
            var top_1 = pixels[row][x];
            var bottom = (_b = (_a = pixels[row + 1]) === null || _a === void 0 ? void 0 : _a[x]) !== null && _b !== void 0 ? _b : top_1;
            line += "".concat(rgb(bottom[0], bottom[1], bottom[2])).concat(rgb(top_1[0], top_1[1], top_1[2], true), "\u2584");
        }
        line += RESET;
        lines.push(line);
    }
    return lines;
}
var DaxnutsComponent = /** @class */ (function () {
    function DaxnutsComponent(ui) {
        this.interval = null;
        this.tick = 0;
        this.maxTicks = 25; // ~2 seconds at 80ms
        this.cachedLines = [];
        this.cachedWidth = 0;
        this.cachedTick = -1;
        this.ui = ui;
        this.image = buildImage();
        this.startAnimation();
    }
    DaxnutsComponent.prototype.invalidate = function () {
        this.cachedWidth = 0;
    };
    DaxnutsComponent.prototype.startAnimation = function () {
        var _this = this;
        this.interval = setInterval(function () {
            _this.tick++;
            if (_this.tick >= _this.maxTicks) {
                _this.stopAnimation();
            }
            _this.cachedWidth = 0;
            _this.ui.requestRender();
        }, 80);
    };
    DaxnutsComponent.prototype.stopAnimation = function () {
        if (this.interval) {
            clearInterval(this.interval);
            this.interval = null;
        }
    };
    DaxnutsComponent.prototype.render = function (width) {
        if (width === this.cachedWidth && this.cachedTick === this.tick) {
            return this.cachedLines;
        }
        var t = theme_js_1.theme;
        var lines = [];
        var center = function (s) {
            var visible = s.replace(/\x1b\[[0-9;]*m/g, "").length;
            var left = Math.max(0, Math.floor((width - visible) / 2));
            return " ".repeat(left) + s;
        };
        lines.push("");
        // Scanline reveal effect: show rows progressively
        var revealedRows = Math.min(this.image.length, Math.floor((this.tick / this.maxTicks) * (this.image.length + 3)));
        for (var i = 0; i < this.image.length; i++) {
            if (i < revealedRows) {
                lines.push(center(this.image[i]));
            }
            else {
                // Show scan line
                if (i === revealedRows) {
                    var scanline = "▓".repeat(WIDTH);
                    lines.push(center(rgb(100, 200, 255) + scanline + RESET));
                }
                else {
                    lines.push(center(" ".repeat(WIDTH)));
                }
            }
        }
        lines.push("");
        // Fade in text after image is revealed
        var textPhase = Math.max(0, this.tick - this.maxTicks * 0.6);
        if (textPhase > 0 || this.tick >= this.maxTicks) {
            lines.push(center(t.fg("accent", "Free Kimi K2.5 via OpenCode Zen")));
            lines.push(center(t.fg("success", '"Powered by daxnuts"')));
            lines.push(center(t.fg("muted", "— @thdxr")));
        }
        else {
            lines.push("");
            lines.push("");
            lines.push("");
        }
        lines.push("");
        if (textPhase > 2 || this.tick >= this.maxTicks) {
            lines.push(center(t.fg("dim", "Try OpenCode")));
            lines.push(center(t.fg("mdLink", "https://mistral.ai/news/mistral-vibe-2-0")));
        }
        else {
            lines.push("");
            lines.push("");
        }
        lines.push("");
        this.cachedLines = lines;
        this.cachedWidth = width;
        this.cachedTick = this.tick;
        return lines;
    };
    DaxnutsComponent.prototype.dispose = function () {
        this.stopAnimation();
    };
    return DaxnutsComponent;
}());
exports.DaxnutsComponent = DaxnutsComponent;
